...
<div class="flex justify-between items-center mb-6">
    ...
    <div class="flex gap-2">
        <a href="{{ route('barangs.create') }}" class="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-md shadow">
            + Tambah Barang
        </a>
        <a href="{{ route('barang-keluar.index') }}" class="bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2 rounded-md shadow">
            📦 Barang Keluar
        </a>
    </div>
</div>
    ...
    @foreach ($barangs as $barang)
        <div class="bg-white border border-gray-200 rounded-xl p-4 shadow-sm hover:shadow-md transition">
            ...
            <div class="text-sm text-gray-600 mb-2">
                ...
                <p><strong>Dibuat oleh:</strong> {{ $barang->created_with }}</p>
                <p><strong>Diupdate oleh:</strong> {{ $barang->updated_with ?? 'Tidak Diketahui' }}</p>
            </div>
            ...
            <div class="flex justify-between mt-4">
                ...
                <button type="button"
                    class="text-sm bg-red-500 text-white px-3 py-1 rounded hover:bg-red-600 btn-hapus"
                    data-action="{{route('barangs.destroy', $barangs->id)}}">
                    📦 Barang Keluar
                </button>
            </div>
        </div>
...
<script>
    ...
    document.querySelectorAll('.btn-hapus').forEach(button => {
        button.addEventListener('click', function () {
            const barangId = this.getAttribute('data-action');
            // const action = `${barangId}`;
